  ...
  if (query.isError) {
    return (
      <div className="rounded-2xl border border-border bg-card p-6 shadow-xl">
        ...
        <Button variant="link" className="mt-3 h-auto px-0 text-sm" onClick={() => query.refetch()}>
          Повторить
        </Button>
      </div>
    );
      ...
      <header className="flex flex-col gap-3 border-b border-border pb-4 md:flex-row md:items-center md:justify-between">
        <div className="min-w-0">
          ...
          <div className="mt-1 flex items-center gap-2">
            ...
            <InfoTooltip text={collection.description || "Описание отсутствует"} />
          </div>
      ...
      <details className="rounded-2xl border border-border bg-card p-4 shadow-lg">
        ...
        <div className="mt-3 grid gap-4 md:grid-cols-2">
          <article className="space-y-1 rounded-2xl border border-border bg-muted/10 p-3">
            <h3 className="flex items-center gap-1 text-xs uppercase text-foreground">
            ...
            <p className="text-sm text-foreground">Количество фрагментов: {collection.vector_repo_info?.points_count ?? "—"}</p>
          </article>
          <article className="space-y-1 rounded-2xl border border-border bg-muted/10 p-3">
            <h3 className="flex items-center gap-1 text-xs uppercase text-foreground">
              <span className="tracking-[0.4em]">
            ...
            <p className="text-sm text-foreground">Количество фрагментов: {collection.keyword_repo_info?.points_count ?? "—"}</p>
          </article>
        </div>
      </details>
